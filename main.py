...
def compare_players(name1: str, tag1: str, name2: str, tag2: str):
    ...
    if not stats1:
        return {"error": f"Player {name1}#{tag1} not found or no competitive matches"}
    if not stats2:
        return {"error": f"Player {name2}#{tag2} not found or no competitive matches"}

    return {
        "player1": {
            "name": f"{name1}#{tag1}",
            **get_overall_stats(stats1)
        },
        "player2": {
            "name": f"{name2}#{tag2}",
            **get_overall_stats(stats2)
        }
    }